...
def init_generation(retriever, model, tokenizer, system_prompt=""):
    ...
    if (system_prompt is None) or (len(system_prompt.strip()) == 0):
        llm_prompt = (
            "Отвечайте на вопрос строго по контексту. Ответ — одной строкой. "
    ...
    full_prompt_template = tokenizer.apply_chat_template(
...
def get_results(
    generation_chain, dataset, skip=None, take=None, write_logs=False, sleep_time=0
):
